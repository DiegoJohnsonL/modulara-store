...
export default function Models({ images }: ModelsProps) {
  ...
  return (
    <section id="models" className="bg-white p-6 md:p-20 max-h-screen">
      <div className="mx-auto flex flex-col gap-6 md:gap-12 justify-center items-stretch h-full">
        ...
        <div className="h-full flex-grow flex gap-4 md:gap-6">
              ...
              <CarouselContent className="h-full m-0">
                {images.map((image, index) => (
                  <CarouselItem className="p-0" key={index}>
                    <img src={image.src} alt={image.alt} className=" w-full max-h-full object-cover" />
                  </CarouselItem>
                ))}
              </CarouselContent>
              <Button
