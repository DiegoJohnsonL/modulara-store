...
export function TypographyH1({ children, className }: Readonly<TypographyProps>) {
  return <h1 className={cn("font-bold text-4xl md:text-7xl", className)}>{children}</h1>;
}
...
export function TypographyH5({ children, className }: Readonly<TypographyProps>) {
  return <h5 className={cn("font-medium text-base md:text-[22px] md:leading-8", className)}>{children}</h5>;
}
